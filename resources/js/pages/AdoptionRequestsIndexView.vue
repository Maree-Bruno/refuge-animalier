...
<template>
    ...
    <!-- Modal de détails (à créer séparément) -->
    <CenterModal v-model="isShowModalOpen" route-key="request" :route-value="selectedRow?.id">
        <div class="p-6">
            <h3 class="text-lg font-semibold mb-4">Détails de la demande</h3>
            <div v-if="selectedRow" class="space-y-3">
                <div>
                    <p class="text-sm text-gray-600">Nom</p>
                    <p class="font-medium">{{ selectedRow.adopter?.name }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-600">Email</p>
                    <p class="font-medium">{{ selectedRow.adopter?.email }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-600">Téléphone</p>
                    <p class="font-medium">{{ selectedRow.adopter?.phone }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-600">Animal</p>
                    <p class="font-medium">{{ selectedRow.animal?.name }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-600">Message</p>
                    <p class="font-medium">{{ selectedRow.message }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-600">Statut</p>
                    <p class="font-medium">{{ getStatusLabel(selectedRow.status) }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-600">Date de demande</p>
                    <p class="font-medium">{{ formatDate(selectedRow.request_date) }}</p>
                </div>
            </div>
        </div>
    </CenterModal>
